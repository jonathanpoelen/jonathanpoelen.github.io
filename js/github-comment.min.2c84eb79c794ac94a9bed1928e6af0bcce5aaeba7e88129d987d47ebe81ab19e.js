let DoGithubComments=(function(){let createComment=function(comment)
...
let XHR=function(url,act){let req=new XMLHttpRequest()
req.onerror=function(event){elist.innerHTML='<p>Erreur de chargement. '+btncomment+'</p>'}
req.onload=function(event){if(this.status===200){act(JSON.parse(event.currentTarget.response))}
else{elist.innerHTML="<p>Les commentaires ne sont pas encore ouverts.</p>"
...
XHR(api_issue_url+issue_id,function(json){const baseurl=api_issue_url+issue_id+"/comments?per_page="+per_page+"&page="
const nb_comment=json.comments
let page=0
if(json.user.id!==1436727){elist.innerHTML+=createComment(json)+btncomment}
console.log(nb_comment)
if(nb_comment>1){let openComments
let extraMessage=''
openComments=function(){++page
XHR(baseurl+page,function(json){elist.innerHTML+=json.reduce(function(s,comment){return s+createComment(comment)},'')+btncomment+extraMessage
ecomments.innerHTML="Charger plus de commentaires (encore "+(nb_comment-((page-1)*per_page+json.length))+")"})
if(page*per_page>=nb_comment){ecomments.style='display: none'
extraMessage='<p>Fin des commentaires</p>'}}
openComments()
ecomments.onclick=function(e){openComments()
e.preventDefault()}}
else{if(json.user.id===1436727){elist.innerHTML+="<div class='gh-comment'>Aucun commentaire pour le moment.</div>"
ecomments.style='display: none'}
