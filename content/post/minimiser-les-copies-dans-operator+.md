...
{
  std::move(lhs) += rhs;
  return std::move(lhs); // ne pas oublier std::move, sinon il y a aura copie en sortie puisque lhs est une référence
}
...
int main()
{
  ...
  test<const Matrix, const Matrix>(); std::cout << "\n\n";
  test<      Matrix, const Matrix>();
}
```
...
Matrix operator +(Lhs && lhs, Rhs && rhs)
{
  ...
  NewLhs new_lhs {
    const_cast<Matrix&>(swap_arg ? rhs : lhs)
  };

  NewRhs new_rhs {
    static_cast<NewRhs>(const_cast<Matrix&>(swap_arg ? lhs : rhs))
  };
...
En première impression `static_cast<NewRhs>` pourrait être optionnel, mais celui-ci permet de forcer la rvalue pour construire NewRhs. Une lvalue (le retour de `const_cast`) ne pouvant être affectée à une rvalue sans cela.
